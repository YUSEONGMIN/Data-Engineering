...
SELECT 	emp_id AS 직원ID, -- emp_id 컬럼에서 조회한 값을 "직원 ID" 컬럼으로 보여달라.
		emp_name "직원 이름", 
        hire_date "입사일",
        salary "급여",
        dept_name "소속부서"
...
-- WHERE	emp_name LIKE '%S'; # S로 끝나는
-- WHERE emp_name LIKE '%S%'; -- S가 들어가는
WHERE emp_name NOT LIKE '%S%'; -- S가 들어가는
...
SELECT * FROM emp WHERE dept_name != 'Sales';
...
SELECT emp_id, emp_name, salary FROM emp WHERE salary BETWEEN 4000.01 AND 7999.9;
...
SELECT emp_id, emp_name, date_format(hire_date,"%Y") YEAR FROM emp WHERE hire_date LIKE "%2004%";
...
SELECT * FROM emp WHERE dept_name NOT IN ('Sales','IT','Shipping');
...
SELECT emp_id, emp_name, job, salary FROM emp WHERE job = 'FI_ACCOUNT' AND salary >= 8000;
...
SELECT emp_id, emp_name, job, hire_date, dept_name FROM emp WHERE job LIKE '%MAN%' AND dept_name = 'Shipping' AND hire_date >= 20050000;

-- TODO: EMP 테이블에서 입사년도가 2004년인 직원들과 (입사년도와 상관없이) 급여가 $20,000 이상인 
--       직원들의 ID(emp_id), 이름(emp_name), 입사일(hire_date), 급여(salary)를 조회.
SELECT emp_id, emp_name, hire_date, salary FROM emp WHERE hire_date LIKE "2004%" OR salary >= 20000;
...
SELECT emp_id, emp_name, job, hire_date, salary FROM emp WHERE job LIKE "%MAN%" AND (salary <= 10000 OR hire_date <= 20000000);
...
ORDER BY 3, 4 DESC, 2 DESC; -- SELECT 절에 선언된 컬럼의 순변 (1부터 시작)
...
SELECT emp_id, emp_name, job, hire_date FROM emp ORDER BY hire_date;
...
SELECT emp_id, emp_name, salary, hire_date FROM emp ORDER BY salary, hire_date;